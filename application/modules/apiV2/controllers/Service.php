...
class Service extends CommonServiceV2 {
		...
		if($this->form_validation->run() == FALSE){
			...
			if($userData){
				...
				if($userData->status == 1){
					...
					$userData->totalFriends = $this->service_model->totalFriendCount($userData->userId);

					$images = $this->service_model->usersImage($userData->userId);
