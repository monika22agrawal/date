...
class Event extends CommonServiceV2 {
    ...
    function createEventNotification_post($eventId,$userId,$eventName,$userName){
        
        $this->Event_model->createEventBgNotification($eventId,$userId,$eventName,$userName);
    ...
    /*function test_shell_get(){
        ...
        echo $userId; exit;
        // send notifications as background process
        shell_exec("php /home/apoimcp/public_html/dev.apoim.com/index.php service event test_message '".$isCreated."' '".$userId."' >> /home/apoimcp/public_html/dev.apoim.com/bgNotification_log.txt &"); 
    }
    ...
    function getEventList_get(){
        ...
        if($data['listType'] == 'myEvent'){
           ...
           $eventData = $this->Event_model->myEventListCount($data);
           
        }elseif($data['listType'] == 'eventRequest'){

            $eventData = $this->Event_model->eventRequestListCount($data);            
        }
    ...
    function getEventDetail_get(){
        ...
        if($data['detailType'] == 'myEvent'){
           ...
           $eventData = $this->Event_model->myEventDetail($data);
            ...
            if(!empty($compId)){
                ...
                $data['compId'] = $compId;
                $eventData = $this->Event_model->sharedEventRequestDetail($data);
                
            }else{

                $data['eventMemId'] = $eventMemId;
                $eventData = $this->Event_model->eventRequestDetail($data);
            }           
        }
      
        if($eventData){
            $response = array('status'=> SUCCESS,'message'=>ResponseMessages::getStatusCodeMessage(200),'currentDate'=>date('Y-m-d H:i:s'),'Detail'=>$eventData['detail'],'joinedMember'=>$eventData['joinedMember'],'invitedMember'=>$eventData['invitedMember'],'companionMember'=>$eventData['companionMember'],'eventReview'=>$eventData['eventReview']);
            $this->response($response); 
        }
        $response = array('status' => FAIL, 'message' => ResponseMessages::getStatusCodeMessage(140));
        $this->response($response);
    ...
    function getEventMemberList_get(){
        ...
        if($data['memberType'] == 'joined'){
           ...
           $eventData = $this->Event_model->joinedMemberCount($data);

        }elseif($data['memberType'] == 'invited'){

            $eventData = $this->Event_model->invitedMemberCount($data);
                       
        }elseif($data['memberType'] == 'companion'){

            $eventData = $this->Event_model->companionMemberCount($data);            
        }        
        ...
        $response = array('status' => SUCCESS, 'message' => ResponseMessages::getStatusCodeMessage(200),'List'=>array(),'eventCreaterDetail'=>$this->Event_model->eventCreaterData($data));
        $this->response($response);
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if($getEventData){
                ...
                if($status == 1){ // for accept request
                    ...
                    $checkUserLimit = $this->Event_model->getEventMemberCount($eventId);
                    ...
                    if($checkUserLimit === TRUE){
                        ...
                        if($isUpdated){
                            ...
                            if($user_info_for){               
                                ...
                                $title = lang('join_event_title');

                                $body_send  = $this->authData->fullName.lang('event_mem_join').$getEventData->eventName.'.'; //body to be sent with current notification
                                $body_save  = '[UNAME]'.lang('event_mem_join').'[ENAME].'; //body to be saved in DB
                ...
                if($checkUserLimit === TRUE){
                    ...
                    if($token){
                        ...
                        if(!empty($result['data']) && $result['status'] === true){*/
                            ...
                            $getOrganiserBankAcc = $this->Event_model->getOrganiserBankAccId($eventId);
                                ...
                                if(!empty($isPaymentDone['data']) && $isPaymentDone['status'] === true){
                                    ...
                                    if($insertId){
                                        ...
                                        if($isUpdated){
                                            ...
                                            if($user_info_for){               
                                                $registrationIds[] = $user_info_for->deviceToken; 
                                                $title = lang('pay_event_title');

                                                $body_send  = $this->authData->fullName.lang('event_paid').$getEventData->eventName.'.'; //body to be sent with current notification
                                                $body_save  = '[UNAME]'.lang('event_paid').'[ENAME].'; //body to be saved in DB
    ...
    // for sending background notifications of multiple users
    function shareEventBgNotification_post($eventMemId,$userId,$eventName,$userName,$eventId){
        
        $this->Event_model->shareEventBgNotification($eventMemId,$userId,$eventName,$userName,$eventId);
                ...
                if($status == 1){ // to accept request 
                    ...
                    if($checkUserLimit === TRUE){
                        ...
                        if($isUpdated){
                            ...
                            if($user_info_for){               
                                $registrationIds[] = $user_info_for->deviceToken; 
                                $title = lang('comp_ac_event_title');

                                $body_send  = $this->authData->fullName.lang('event_accepted'); //body to be sent with current notification
                                $body_save  = '[UNAME]'.lang('event_accepted'); //body to be saved in DB
                    ...
                    if($isUpdated){
                        ...
                        if($user_info_for){               
                            $registrationIds[] = $user_info_for->deviceToken; 
                            $title = lang('comp_rej_event_title');

                            $body_send  = $this->authData->fullName.lang('event_rejected'); //body to be sent with current notification
                            $body_save  = '[UNAME]'.lang('event_rejected'); //body to be saved in DB
                                    ...
                                    if($insertId){
                                        ...
                                        if($isUpdated){
                                            ...
                                            if($user_info_for){               
                                                ...
                                                $body_send  = $this->authData->fullName.lang('event_paid_comp'); //body to be sent with current notification
                                                $body_save  = '[UNAME]'.lang('event_paid_comp'); //body to be saved in DB
    ...
    function getBankAccDetail_get(){
        ...
        $userId = $this->authData->userId;        
        //get data
        $accountDetail = $this->Event_model->getBankAccountDetail($userId);
        if($accountDetail){
