...
class Appointment extends CommonServiceV2 {
           ...
           /* if($type != $data['offerType']){ // if offerType and appointment for user 
            ...
            if($appoimData){
                ...
                if($user_info_for->isNotification == 1){
                    ...
                    $title = lang('new_app_title');

                    $body_send  = $this->authData->fullName.lang('create_app'); //body to be sent with current notification
                    $body_save  = '[UNAME]'.lang('create_app');; //body to be saved in DB
    ...
    function updateAppointment_post(){
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if($appointData){
                ...
                if($user_info_for){               
                    $webToken = $registrationIds[] = $user_info_for->deviceToken;
                    $title = lang('update_appointment');

                    $body_send  = $this->session->userdata('fullName').lang('update_app'); //body to be sent with current notification
                    $body_save  = '[UNAME]'.lang('update_app'); //body to be saved in DB
    ...
    function getAppointmentDetail_get(){
        ...
        //get data
        $appoimData = $this->Appointment_model->getAppData($appId,$userId);
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if($appointData){
                ...
                if($user_info_for){               
                    ...
                    $body_send  = $this->authData->fullName.lang('apply_counter_app'); //body to be sent with current notification
                    $body_save  = '[UNAME]'.lang('apply_counter_app'); //body to be saved in DB
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if($appointData){
                ...
                if($user_info_for){
                    ...
                    $status = ($counterStatus == 1) ? lang('accepted'): lang('rejected');            
                    $registrationIds[] = $user_info_for->deviceToken;
                    $title = lang('counter_app_title');

                    $body_send  = $this->authData->fullName.lang('has').$status.lang('app_counter'); //body to be sent with current notification
                    $body_save  = '[UNAME]'.lang('has').$status.lang('app_counter'); //body to be saved in DB
	...
	function changeAppointmentStatus_post(){
        ...
        if($appointData){
            ...
            $user_info_for = $this->Appointment_model->getByDeviceToken($appId,$userId);

            if($user_info_for){
                         
                $registrationIds[] = $user_info_for->deviceToken; 
                
                $title = lang('appointment_confirmed');

                $body_send  = $this->authData->fullName.$msg; //body to be sent with current notification
                $body_save  = '[UNAME]'.$msg; //body to be saved in DB
    ...
    function appointmentPayment_post(){
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $getOrganiserBankAcc = $this->Appointment_model->getBankAccountDetail($appointForId);
                    ...
                    if($insertId){
                        ...
                        if($isUpdated){
                            ...
                            if($user_info_for){ 
                                ...
                                $registrationIds[] = $user_info_for->deviceToken; 
                                $title = lang('pay_app_title');

                                $body_send  = $this->authData->fullName.lang('app_paid'); //body to be sent with current notification
                                $body_save  = '[UNAME]'.lang('app_paid'); //body to be saved in DB
	...
	function finishAppointment_post(){
        ...
        if($appointData){
            ...
            $user_info_for = $this->Appointment_model->getDeviceToken($appId,$userId);
            if($user_info_for){               
                $registrationIds[] = $user_info_for->deviceToken; 
                $title = "Finish Appointment";

                $body_send  = $this->authData->fullName.' '.lang('app_finished'); //body to be sent with current notification
                $body_save  = '[UNAME] '.lang('app_finished'); //body to be saved in DB
	...
	function deleteAppointment_post(){
        ...
        if($user_info_for){
            ...
            $title = ($type == 'deleted') ? lang('del_app_title') : lang('cncl_app_title');
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if($appoimData){
                ...
                if($user_info_for){
                    ...
                    $registrationIds[] = $user_info_for->deviceToken;
                    $title = ($data['reviewType'] == 1) ? lang('rev_app_title') : lang('event_review');

                    $body_send  = $this->authData->fullName.lang('app_give_review'); //body to be sent with current notification
                    $body_save  = '[UNAME]'.lang('app_give_review'); //body to be saved in DB
