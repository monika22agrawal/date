...
class User extends CommonService {
				...
				if(is_array($verifyNo)){
					...
					switch ($verifyNo['status']) {
						...
						case "0":
							...
							$responseArray = array('status'=>FAIL,'message'=>ResponseMessages::getStatusCodeMessage(196));
							break;
	...
	function updateProfile_post(){
		...
		if(!empty($this->post('eventInvitation'))){
	...
	// update notification status for on / off
	function setUserLanguage_post(){
		//check for auth
		if(!$this->check_service_auth()){
            $this->response($this->token_error_msg(), SERVER_ERROR);  //authetication failed
        }
        $user_id 	= $this->authData->userId;
        $setLanguage 	= $this->post('setLanguage');
      
        $where = array('userId'=>$user_id);        	
        //update status
        $result = $this->common_model->updateFields(USERS, array('setLanguage'=>$setLanguage),$where);
    	//check for data delete yes or not
    	if($result){
    		$response = array('status' => SUCCESS, 'message' => ResponseMessages::getStatusCodeMessage(154));
    		$this->response($response);
