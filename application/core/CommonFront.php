...
class CommonFront extends MX_Controller {
    ...
    function check_user_session() {
        ...
        $is_exists = $this->common_model->is_data_exists(USERS, array('userId'=>$this->session->userdata('userId'),'status'=>1));
        if(!$is_exists){
            $this->session->sess_destroy();
            redirect(site_url('home/logout'));
        }
    ...
    //authentication for ajax
    function check_ajax_auth(){
        $failed_res = json_encode(array('status'=>-1,'msg'=>'Your session expired. Please login again.','url'=>site_url('home/login')));
        if(empty($this->session->userdata('userId'))){
            return $failed_res;
        }
   ...
   public function image_unlink($filename, $filepath) {
    ...
    function load_language_files(){
        $language_array = array('english','spanish');//allow language 

        $this->appLang = 'english'; //set default langauge
        $userLanguage = $this->session->userdata('language'); //get user session data in key
        $language = '';
        if(!empty($userLanguage)){//check if language key set or not  in case of logout 
            $language = $userLanguage;//get language from session key 
        }

        if(!empty($language) && in_array($language,$language_array)){//check language key set or not and language is in language array 
            $this->appLang = $language; //set user langauge
        }
        ...
        $this->lang->load('home_message_lang', $this->appLang);  //load response lang file
        ...
        switch ($current_control) {//case can be added according to requirment
            case 'login':
                $this->lang->load('login_signup_message_lang', $this->appLang); //load response lang file
                break;
            case 'registration':
                $this->lang->load('login_signup_message_lang', $this->appLang); //load response lang file
                break;
